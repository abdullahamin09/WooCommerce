...
const Card = ({ Title, Price, discountPrice, Ratings, Description, Image }) => {
        ...
        <div className="main-card  xl:max-w-[285px] max-sm:max-w-[350px]">
            ...
            <div className="card-details">
                <div className="det-wrap p-[20px] bg-[#EEEEEE]">
                    <p className='font-bold text-[20px]/[30px] cursor-pointer'><Link to="/singleproduct">{Title}</Link></p>
                    {Ratings &&
                    ...
                    <div className="flex flex-row gap-2 text-[20px]/[30px]">
                        <span className={` ${discountPrice ? "line-through font-normal" : "font-bold"} `} >R{Price}</span>
                        {discountPrice && (
                            <span className="font-bold">R{discountPrice}</span>
                        )}
                        <span className='font-medium'>incl VAT</span>
                    </div>
                    <p className='font-normal text-[16px]/[24px]'>{Description}</p>
                </div>
